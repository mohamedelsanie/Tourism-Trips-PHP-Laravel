<x-admin-layout>
                ...
                <div class="col-md-6 col-sm-12">
                    <nav aria-label="breadcrumb" role="navigation">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">{{ __('admin/offer.edit.edit') }}<code>{{ $page->title }}</code></li>
                        </ol>
                    </nav>
                </div>
        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 text-gray-900">
                <div class="clearfix mb-10">
                    <div class="pull-left">
                        <h4 class="text-blue h4">{{ __('admin/offer.edit.edit') }} </h4>
                    </div>
                </div>
                <div class="dropdown-divider"></div>
                <form action="{{ route('admin.offers.update', $page->id) }}" method="post">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="form-group row">
                        <div class="col-sm-12 col-md-8 mb-30">
                                    ...
                                    <div class="tab-content">
                                        @foreach(config('translatable.languages') as $key => $lang)
                                            <div class="tab-pane @if(LaravelLocalization::getCurrentLocale() == $key) active @endif" id="title-{{ $key }}" role="tabpanel">
                                                <input name="{{ $key }}[title]" placeholder="{{ __('admin/offer.fields.title') }}" value="{{ $page->translate($key)->title }}" class="@if(LaravelLocalization::getCurrentLocale() == $key) slug-input main_input @endif border-gray-300 rounded-md shadow-sm form-control @if($errors->has($key.'*title')) border border-danger @endif" type="text"/>
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-sm-12 col-md-12">
                                    <input name="slug" placeholder="{{ __('admin/offer.fields.slug') }}" value="{{ $page->slug }}" class="slug-output border-gray-300 rounded-md shadow-sm form-control @error('slug') border border-danger @enderror" type="text"/>
                                    <span class="">{{ __('admin/offer.fields.current_slug') }}{{ getSetting('site_url') }}<b class="text-success">{{ $page->slug }}</b></span>
                                            ...
                                            <div class="tab-content">
                                                @foreach(config('translatable.languages') as $key => $lang)
                                                    <div class="tab-pane @if(LaravelLocalization::getCurrentLocale() == $key) active @endif" id="description-{{ $key }}" role="tabpanel">
                                                        <textarea name="{{ $key }}[description]" placeholder="{{ __('admin/offer.fields.description') }}" class="border-gray-300 rounded-md shadow-sm form-control @if($errors->has($key.'*description')) border border-danger @endif">{{ $page->translate($key)->description }}</textarea>
                                            ...
                                            <div class="tab-content">
                                                @foreach(config('translatable.languages') as $key => $lang)
                                                    <div class="tab-pane @if(LaravelLocalization::getCurrentLocale() == $key) active @endif" id="content-{{ $key }}" role="tabpanel">
                                                        <textarea name="{{ $key }}[content]" placeholder="{{ __('admin/offer.fields.content') }}" id="editor_{{ $key }}" class="0textarea_editor_{{ $key }} border-gray-300 rounded-md shadow-sm form-control @if($errors->has($key.'*content')) border border-danger @endif">{{ $page->translate($key)->content }}</textarea>
                        ...
                        <div class="col-sm-12 col-md-4 mb-30">
                            ...
                            <div class="card card-box custom mb-10" id="accordionPrice">
                                ...
                                <div id="collapsePrice" class="card-body show" data-parent="#accordionPrice" >
                                    <div class="form-group row">
                                        <div class="col-sm-12 col-md-12">
                                            <input name="price" placeholder="{{ __('admin/offer.fields.price') }}" value="{{ $page->price }}" class="border-gray-300 rounded-md shadow-sm form-control @error('price') border border-danger @enderror" type="text"/>
                        ...
                        <div class="col-sm-12 col-md-10">
                            <button class="btn btn-primary bg-gray-800" type="submit">{{ __('admin/offer.fields.update') }}</button>
                        </div>
                    </div>
                </form>
    ...
    @endsection
    @section('page_title'){{ __('admin/offer.edit.title_tag',['page' => $page->title]) }}@endsection
</x-admin-layout>
