<x-admin-layout>
                ...
                <div class="col-md-6 col-sm-12">
                    <nav aria-label="breadcrumb" role="navigation">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">{{ __('admin/tour.create.create') }}</li>
                        </ol>
                    </nav>
                </div>
        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 text-gray-900">
                <div class="clearfix mb-10">
                    <div class="pull-left">
                        <h4 class="text-blue h4">{{ __('admin/tour.create.create') }}</h4>
                    </div>
                </div>
                <div class="dropdown-divider"></div>

                <form action="{{ route('admin.tours.store') }}" method="post" class="mt-6 space-y-6">
                    @csrf
                    ...
                    <div class="form-group row">
                        <div class="col-sm-12 col-md-8 mb-30">
                            ...
                            <div class="form-group row">
                                <label class="col-sm-12 col-md-12 col-form-label">{{ __('admin/tour.fields.title') }}</label>
                                    ...
                                    <div class="tab-content">
                                        @foreach(config('translatable.languages') as $key => $lang)
                                            <div class="tab-pane @if(LaravelLocalization::getCurrentLocale() == $key) active @endif" id="title-{{ $key }}" role="tabpanel">
                                                <input name="{{ $key }}[title]" placeholder="{{ __('admin/tour.fields.title') }}" value="{{ old($key.'.title') }}" class="@if(LaravelLocalization::getCurrentLocale() == $key) slug-input main_input @endif border-gray-300 rounded-md shadow-sm form-control @if($errors->has($key.'*title')) border border-danger @endif" type="text"/>
                            ...
                            <div class="form-group row">
                                <label class="col-sm-12 col-md-12 col-form-label">{{ __('admin/tour.fields.slug') }}</label>
                                <div class="col-sm-12 col-md-12">
                                    <input name="slug" placeholder="{{ __('admin/tour.fields.slug') }}" value="{{ old('slug') }}" class="slug-output border-gray-300 rounded-md shadow-sm form-control @error('slug') border border-danger @enderror" type="text"/>
                            ...
                            <div class="card card-box custom mb-10" id="accordionWork">
                                ...
                                <div id="collapseWork" class="card-body show" data-parent="#accordionWork" >
                                            ...
                                            <div class="tab-content">
                                                @foreach(config('translatable.languages') as $key => $lang)
                                                    <div class="tab-pane @if(LaravelLocalization::getCurrentLocale() == $key) active @endif" id="description-{{ $key }}" role="tabpanel">
                                                        <textarea name="{{ $key }}[description]" placeholder="{{ __('admin/tour.fields.description') }}" class="border-gray-300 rounded-md shadow-sm form-control @if($errors->has($key.'*description')) border border-danger @endif">{{ old($key.'.description') }}</textarea>
                                            ...
                                            <div class="tab-content">
                                                @foreach(config('translatable.languages') as $key => $lang)
                                                    <div class="tab-pane @if(LaravelLocalization::getCurrentLocale() == $key) active @endif" id="content-{{ $key }}" role="tabpanel">
                                                        <textarea name="{{ $key }}[content]" placeholder="{{ __('admin/tour.fields.content') }}" id="editor_{{ $key }}" class="0textarea_editor_{{ $key }} border-gray-300 rounded-md shadow-sm form-control @if($errors->has($key.'*content')) border border-danger @endif">{{ old($key.'.content') }}</textarea>
                                            ...
                                            <div class="tab-content">
                                                @foreach(config('translatable.languages') as $key => $lang)
                                                    <div class="tab-pane @if(LaravelLocalization::getCurrentLocale() == $key) active @endif" id="from_place-{{ $key }}" role="tabpanel">
                                                        <input name="{{ $key }}[from_place]" placeholder="{{ __('admin/tour.fields.from_place') }}" value="{{ old($key.'.from_place') }}" class="border-gray-300 rounded-md shadow-sm form-control @if($errors->has($key.'*from_place')) border border-danger @endif" type="text"/>
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-sm-12 col-md-12">
                                            <input name="from_date" placeholder="{{ __('admin/tour.fields.from_date') }}" value="{{ old('from_date') }}" class="date-picker2 border-gray-300 rounded-md shadow-sm form-control @error('from_date') border border-danger @enderror" type="text"/>
                                            ...
                                            <div class="tab-content">
                                                @foreach(config('translatable.languages') as $key => $lang)
                                                    <div class="tab-pane @if(LaravelLocalization::getCurrentLocale() == $key) active @endif" id="to_place-{{ $key }}" role="tabpanel">
                                                        <input name="{{ $key }}[to_place]" placeholder="{{ __('admin/tour.fields.to_place') }}" value="{{ old($key.'.to_place') }}" class="border-gray-300 rounded-md shadow-sm form-control @if($errors->has($key.'*to_place')) border border-danger @endif" type="text"/>
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-sm-12 col-md-12">
                                            <input name="to_date" placeholder="{{ __('admin/tour.fields.to_date') }}" value="{{ old('to_date') }}" class="date-picker2 border-gray-300 rounded-md shadow-sm form-control @error('to_date') border border-danger @enderror" type="text"/>
                        ...
                        <div class="col-sm-12 col-md-4 mb-30">
                            <div class="card card-box custom mb-10" id="accordionPrice">
                                ...
                                <div id="collapsePrice" class="card-body show pb-0" data-parent="#accordionPrice" aria-expanded="true">
                                    ...
                                    <div class="form-group row">
                                        <div class="col-sm-12 col-md-12">
                                            <input name="price" placeholder="{{ __('admin/tour.fields.price') }}" value="{{ old('price') }}" class="border-gray-300 rounded-md shadow-sm form-control @error('price') border border-danger @enderror" type="text"/>
                                    ...
                                    <div class="form-group row">
                                        <div class="col-sm-12 col-md-12">
                                            <input name="price_eg" placeholder="{{ __('admin/tour.fields.price_eg') }}" value="{{ old('price_eg') }}" class="border-gray-300 rounded-md shadow-sm form-control @error('price_eg') border border-danger @enderror" type="text"/>
                                    ...
                                    <div class="form-group row">
                                        <div class="col-sm-12 col-md-12">
                                            <select name="category_id" class="border-gray-300 rounded-md shadow-sm custom-select col-12 @error('video_id') border border-danger @enderror">
                                                <option value="" @if(old('category_id') == '') selected @endif>{{ __('admin/video_comment.fields.choose') }}</option>
                                    ...
                                    <div class="form-group row">
                                        <div class="col-sm-12 col-md-12">
                                            <select name="comments_status" class="border-gray-300 rounded-md shadow-sm custom-select col-12 @error('comments_status') border border-danger @enderror">
                                                ...
                                                <option value="open" @if(old('comments_status') == 'open') selected @endif>{{ __('admin/tour.fields.open') }}</option>
                                                <option value="closed" @if(old('comments_status') == 'closed') selected @endif>{{ __('admin/tour.fields.closed') }}</option>
                                            </select>
                                    ...
                                    <div class="form-group row">
                                        <div class="col-sm-12 col-md-12">
                                            <select name="status" class="border-gray-300 rounded-md shadow-sm custom-select col-12 @error('status') border border-danger @enderror">
                                                <option value="" @if(old('status') == '') selected @endif>{{ __('admin/tour.fields.choose') }}</option>
                                                <option value="publish" @if(old('status') == 'publish') selected @endif>{{ __('admin/tour.fields.publish') }}</option>
                                                <option value="pending" @if(old('status') == 'pending') selected @endif>{{ __('admin/tour.fields.pending') }}</option>
                                                <option value="draft" @if(old('status') == 'draft') selected @endif>{{ __('admin/tour.fields.draft') }}</option>
                                            </select>
                    ...
                    <div class="col-sm-12 col-md-10">
                        <button class="btn btn-primary bg-gray-800" type="submit">{{ __('admin/tour.fields.create') }}</button>
                    </div>
            </div>
            </form>
    ...
    @endsection
    @section('page_title'){{ __('admin/tour.create.title_tag') }}@endsection
</x-admin-layout>
