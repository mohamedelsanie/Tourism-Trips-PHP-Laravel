                    ...
                    <nav aria-label="breadcrumb" role="navigation">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">
                                {{ __('admin/tour.index.title') }}
                            </li>
                        </ol>
                    </nav>
                </div>
                <div class="col-md-6 col-sm-12 text-right">
                    @if(AdminCan('tour-create'))
                    <a href="{{ route('admin.tours.create') }}" class="btn btn-primary btn-sm scroll-click"><i class="fa fa-plus"></i> {{ __('admin/tour.index.create') }}</a>
                    @endif
                    @if(AdminCan('tour-forcedelete'))
                    <a href="{{ route('admin.tours.archive') }}" class="btn btn-primary btn-sm scroll-click">{{ __('admin/tour.index.archive') }}</a>
                    @endif
                </div>
            </div>
        </div>
    ...
    <div class="py-12">
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 text-gray-900">
                <div class="row">
                    <div class="col-md-6 col-sm-12">
                        <div class="clearfix mb-10">
                            <div class="pull-left">
                                <h4 class="text-blue h4">{{ __('admin/tour.index.title') }}</h4>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="col-md-6 col-sm-12">
                        ...
                        <form name="search_form" method="post" action="{{route('admin.tours.search')}}" >
                            @csrf
                ...
                <form name="destroy_all" method="post" action="{{route('admin.tours.destroy_all')}}" >
                    @csrf
                        ...
                        @if(count($pages) > 0)
                            @foreach($pages as $page)
                                <tr>
                                    ...
                                    <td><span class="badge badge-pill" data-bgcolor="#e7ebf5" data-color="#265ed7" style="color: rgb(38, 94, 215); background-color: rgb(231, 235, 245);">{{ $page->status }}</span></td>

                                    <td>
                                        <div class="table-actions">
                                            @if(AdminCan('tour-edit'))
                                                <a href="{{ route('admin.tours.edit',$page->id) }}" data-color="#265ed7" style="color: rgb(38, 94, 215);"><i class="icon-copy dw dw-edit2"></i></a>
                                            @endif
                                            @if(AdminCan('tour-delete'))
                                                    ...
                                                    <x-modal name="confirm-deletion_{{ $page->id }}" focusable>
                                                        <div class="p-6">
                                                            ...
                                                            <div class="mt-6 flex justify-end">
                                                                ...
                                                                <a href="{{ route('admin.tours.delete',$page->id) }}" class="inline-flex items-center px-4 py-2 bg-red-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-red-500 active:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 transition ease-in-out duration-150 ml-3">
                                                                    {{ __('admin/common.messages.confirm_delete') }}
                                                                </a>
                                                            </div>
                                                        </div>
                                                    </x-modal>
{{--                                                <a href="{{ route('admin.tours.delete',$page->id) }}" data-color="#e95959" style="color: rgb(233, 89, 89);"><i class="icon-copy dw dw-delete-3"></i></a>--}}
                                            @endif
                                                <a href="{{ route('tour',$page->slug) }}" data-color="#265ed7" style="color: rgb(38, 94, 215);"><i class="dw dw-eye"></i></a>
                                        </div>
                                    </td>
                                </tr>
                            @endforeach
                        @else
                            <tr>
                                <td colspan="4">{{ __('admin/tour.index.notfound') }}</td>
                            </tr>
                        @endif
                        </tbody>
                    </table>
    ...
    @section('page_title'){{ __('admin/tour.index.title_tag') }}@endsection
</x-admin-layout>
