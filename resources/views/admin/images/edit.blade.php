<x-admin-layout>
                ...
                <div class="col-md-6 col-sm-12">
                    <nav aria-label="breadcrumb" role="navigation">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">{{ __('admin/image.edit.edit') }}<code>{{ $ad->title }}</code></li>
                        </ol>
                    </nav>
                </div>
        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 text-gray-900">
                <div class="clearfix mb-10">
                    <div class="pull-left">
                        <h4 class="text-blue h4">{{ __('admin/image.edit.edit') }} </h4>
                    </div>
                </div>
                <div class="dropdown-divider"></div>

                <form action="{{ route('admin.images.update', $ad->id) }}" method="post">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="form-group row">
                        <div class="col-sm-12 col-md-8 mb-30">
                                    ...
                                    <div class="tab-content">
                                        @foreach(config('translatable.languages') as $key => $lang)
                                            <div class="tab-pane @if(LaravelLocalization::getCurrentLocale() == $key) active @endif" id="title-{{ $key }}" role="tabpanel">
                                                <input name="{{ $key }}[title]" placeholder="{{ __('admin/image.fields.title') }}" value="{{ $ad->translate($key)->title }}" class="@if(LaravelLocalization::getCurrentLocale() == $key) slug-input main_input @endif border-gray-300 rounded-md shadow-sm form-control @if($errors->has($key.'*title')) border border-danger @endif" type="text"/>
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-sm-12 col-md-12">
                                    <input name="slug" placeholder="{{ __('admin/image.fields.slug') }}" value="{{ $ad->slug }}" class="slug-output border-gray-300 rounded-md shadow-sm form-control @error('slug') border border-danger @enderror" type="text"/>
                                    <span class="">{{ __('admin/image.fields.current_slug') }}{{ getSetting('site_url') }}<b class="text-success">{{ $ad->slug }}</b></span>
                                            ...
                                            <div class="tab-content">
                                                @foreach(config('translatable.languages') as $key => $lang)
                                                    <div class="tab-pane @if(LaravelLocalization::getCurrentLocale() == $key) active @endif" id="description-{{ $key }}" role="tabpanel">
                                                        <textarea name="{{ $key }}[description]" placeholder="{{ __('admin/image.fields.description') }}" class="border-gray-300 rounded-md shadow-sm form-control @if($errors->has($key.'*description')) border border-danger @endif">{{ $ad->translate($key)->description }}</textarea>
                                            ...
                                            <div class="tab-content">
                                                @foreach(config('translatable.languages') as $key => $lang)
                                                    <div class="tab-pane @if(LaravelLocalization::getCurrentLocale() == $key) active @endif" id="content-{{ $key }}" role="tabpanel">
                                                        <textarea name="{{ $key }}[content]" placeholder="{{ __('admin/image.fields.content') }}" id="editor_{{ $key }}" class="0textarea_editor_{{ $key }} border-gray-300 rounded-md shadow-sm form-control @if($errors->has($key.'*content')) border border-danger @endif">{{ $ad->translate($key)->content }}</textarea>
                                                    ...
                                                    <table class="table table-bordered" id="gallery_table">
                                                        ...
                                                        @if(!empty($ad->gallery['link']))
                                                            @foreach($ad->gallery['link'] as $gal)
                                                                <tr>
                                                                    <td>
                                                                        <input type="text" placeholder="{{ __('admin/image.fields.gallery_link') }}" value="{{ $gal }}" name="gallery[link][]" class="border-gray-300 rounded-md shadow-sm form-control" />
                                                                    </td>
                                        ...
                                        <div class="col-sm-12 col-md-12">
                                            <select name="status" class="border-gray-300 rounded-md shadow-sm custom-select col-12 @error('status') border border-danger @enderror">
                                                @if($ad->status == 'publish')
                                                    <option value="publish" selected>{{ __('admin/image.fields.publish') }}</option>
                                                    <option value="pending">{{ __('admin/image.fields.pending') }}</option>
                                                    <option value="draft">{{ __('admin/image.fields.draft') }}</option>
                                                @elseif($ad->status == 'pending')
                                                    <option value="publish">{{ __('admin/image.fields.publish') }}</option>
                                                    <option value="pending" selected>{{ __('admin/image.fields.pending') }}</option>
                                                    <option value="draft">{{ __('admin/image.fields.draft') }}</option>
                                                @elseif($ad->status == 'draft')
                                                    <option value="publish">{{ __('admin/image.fields.publish') }}</option>
                                                    <option value="pending">{{ __('admin/image.fields.pending') }}</option>
                                                    <option value="draft" selected>{{ __('admin/image.fields.draft') }}</option>
                                                @else
                                                    <option selected="">{{ __('admin/image.fields.choose') }}</option>
                                                    <option value="publish">{{ __('admin/image.fields.publish') }}</option>
                                                    <option value="pending">{{ __('admin/image.fields.pending') }}</option>
                                                    <option value="draft">{{ __('admin/image.fields.draft') }}</option>
                                                @endif
                                        ...
                                        <div class="col-sm-12 col-md-12">
                                            <select name="comments_status" class="border-gray-300 rounded-md shadow-sm custom-select col-12 @error('comments_status') border border-danger @enderror">
                                                @if($ad->comments_status == 'open')
                                                    <option value="open" selected>{{ __('admin/image.fields.open') }}</option>
                                                    <option value="closed">{{ __('admin/image.fields.closed') }}</option>
                                                @elseif($ad->comments_status == 'closed')
                                                    <option value="open">{{ __('admin/image.fields.open') }}</option>
                                                    <option value="closed" selected>{{ __('admin/image.fields.closed') }}</option>
                                                @else
                                                    <option selected="">{{ __('admin/image.fields.choose') }}</option>
                                                    <option value="open">{{ __('admin/image.fields.open') }}</option>
                                                    <option value="closed">{{ __('admin/image.fields.closed') }}</option>
                        ...
                        <div class="col-sm-12 col-md-10">
                            <button class="btn btn-primary bg-gray-800" type="submit">{{ __('admin/image.fields.update') }}</button>
                        </div>
                    </div>
                </form>
    ...
    @endsection
    @section('page_title'){{ __('admin/image.edit.title_tag',['ad' => $ad->title]) }}@endsection
</x-admin-layout>
