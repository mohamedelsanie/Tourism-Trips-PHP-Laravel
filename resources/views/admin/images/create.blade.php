<x-admin-layout>
                ...
                <div class="col-md-6 col-sm-12">
                    <nav aria-label="breadcrumb" role="navigation">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item"><a href="{{ route('admin.images.index') }}">{{ __('admin/image.index.title') }}</a></li>
                            <li class="breadcrumb-item active" aria-current="page">{{ __('admin/image.create.create') }}</li>
                        </ol>
                    </nav>
                </div>
                <div class="col-md-6 col-sm-12 text-right">
                    <a href="{{ route('admin.images.index') }}" class="btn btn-primary btn-sm scroll-click">{{ __('admin/image.show.back') }}</a>
                </div>
            </div>
        </div>
    </x-slot>
        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 text-gray-900">
                <div class="clearfix mb-10">
                    <div class="pull-left">
                        <h4 class="text-blue h4">{{ __('admin/image.create.create') }}</h4>
                    </div>
                </div>
                <div class="dropdown-divider"></div>

                <form action="{{ route('admin.images.store') }}" method="post" class="mt-6 space-y-6">
                    @csrf
                    ...
                    <div class="form-group row">
                        <div class="col-sm-12 col-md-8 mb-30">
                            ...
                            <div class="form-group row">
                                <label class="col-sm-12 col-md-12 col-form-label">{{ __('admin/image.fields.title') }}</label>
                                    ...
                                    <div class="tab-content">
                                        @foreach(config('translatable.languages') as $key => $lang)
                                            <div class="tab-pane @if(LaravelLocalization::getCurrentLocale() == $key) active @endif" id="title-{{ $key }}" role="tabpanel">
                                                <input name="{{ $key }}[title]" placeholder="{{ __('admin/image.fields.title') }}" value="{{ old($key.'.title') }}" class="@if(LaravelLocalization::getCurrentLocale() == $key) slug-input main_input @endif border-gray-300 rounded-md shadow-sm form-control @if($errors->has($key.'*title')) border border-danger @endif" type="text"/>
                            ...
                            <div class="form-group row">
                                <label class="col-sm-12 col-md-12 col-form-label">{{ __('admin/image.fields.slug') }}</label>
                                <div class="col-sm-12 col-md-12">
                                    <input name="slug" placeholder="{{ __('admin/image.fields.slug') }}" value="{{ old('slug') }}" class="slug-output border-gray-300 rounded-md shadow-sm form-control @error('slug') border border-danger @enderror" type="text"/>
                            ...
                            <div class="card card-box custom mb-10" id="accordionWork">
                                <div class="card-header" data-toggle="collapse" href="#collapseWork">
                                    <a class="card-title">
                                        {{ __('admin/image.fields.info') }}
                                    </a>
                                </div>
                                <div id="collapseWork" class="card-body show" data-parent="#accordionWork" >

                                    <div class="form-group row">
                                        <label class="col-sm-12 col-md-12 col-form-label">{{ __('admin/image.fields.description') }}</label>
                                        <div class="col-sm-12 col-md-12">
                                            <ul class="nav nav-tabs" role="tablist">
                                                @foreach(config('translatable.languages') as $key => $lang)
                                            ...
                                            <div class="tab-content">
                                                @foreach(config('translatable.languages') as $key => $lang)
                                                    <div class="tab-pane @if(LaravelLocalization::getCurrentLocale() == $key) active @endif" id="description-{{ $key }}" role="tabpanel">
                                                        <textarea name="{{ $key }}[description]" placeholder="{{ __('admin/image.fields.description') }}" class="border-gray-300 rounded-md shadow-sm form-control @if($errors->has($key.'*description')) border border-danger @endif">{{ old($key.'.description') }}</textarea>
                                    ...
                                    <div class="form-group row">
                                        <label class="col-sm-12 col-md-12 col-form-label">{{ __('admin/image.fields.content') }}</label>
                                        <div class="col-sm-12 col-md-12">
                                            <ul class="nav nav-tabs" role="tablist">
                                                @foreach(config('translatable.languages') as $key => $lang)
                                            ...
                                            <div class="tab-content">
                                                @foreach(config('translatable.languages') as $key => $lang)
                                                    <div class="tab-pane @if(LaravelLocalization::getCurrentLocale() == $key) active @endif" id="content-{{ $key }}" role="tabpanel">
                                                        <textarea name="{{ $key }}[content]" placeholder="{{ __('admin/image.fields.content') }}" id="editor_{{ $key }}" class="0textarea_editor_{{ $key }} border-gray-300 rounded-md shadow-sm form-control @if($errors->has($key.'*content')) border border-danger @endif">{{ old($key.'.content') }}</textarea>
                                            ...
                                            <div class="form-group row">
                                                <div class="col-sm-12 col-md-12">
                                                    <table class="table table-bordered" id="gallery_table">
                                                        <tr>
                                                            <th>{{ __('admin/image.fields.gallery_link') }}</th>
                                                            <th><button type="button" class="btn btn-success btn-sm add_watch" style="background: green;"><span class="fa fa-plus"></span></button></th>
                                                        </tr>
                                                    </table>
                        ...
                        <div class="col-sm-12 col-md-4 mb-30">
                                ...
                                <div id="collapseImage" class="card-body show pb-0" data-parent="#accordionImage" aria-expanded="true">
                                    <div class="form-group row" id="user_image_field_{{$field}}">
                                        ...
                                        <div class="col-sm-12 col-md-12">
                                            ...
                                            <div class="block">
                                                <a href="#" class="btn-block" data-toggle="modal" data-target=".media_uploader_{{$field}}" type="button">{{ __('admin/image.fields.media') }}</a>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div class="form-group row">
                                        <div class="col-sm-12 col-md-12">
                                            <select name="status" class="border-gray-300 rounded-md shadow-sm custom-select col-12 @error('status') border border-danger @enderror">
                                                <option value="" @if(old('status') == '') selected @endif>{{ __('admin/image.fields.choose') }}</option>
                                                <option value="publish" @if(old('status') == 'publish') selected @endif>{{ __('admin/image.fields.publish') }}</option>
                                                <option value="pending" @if(old('status') == 'pending') selected @endif>{{ __('admin/image.fields.pending') }}</option>
                                                <option value="draft" @if(old('status') == 'draft') selected @endif>{{ __('admin/image.fields.draft') }}</option>
                                            </select>
                            ...
                            <div class="card card-box custom mb-10" id="accordionCommentsStatus">
                                <div class="card-header" data-toggle="collapse" href="#collapseCommentsStatus">
                                    <a class="card-title">
                                        {{ __('admin/image.fields.comments_status') }}
                                    </a>
                                </div>
                                    ...
                                    <div class="form-group row">
                                        <div class="col-sm-12 col-md-12">
                                            <select name="comments_status" class="border-gray-300 rounded-md shadow-sm custom-select col-12 @error('comments_status') border border-danger @enderror">
                                                <option value="" @if(old('comments_status') == '') selected @endif>{{ __('admin/image.fields.choose') }}</option>
                                                <option value="open" @if(old('comments_status') == 'open') selected @endif>{{ __('admin/image.fields.open') }}</option>
                                                <option value="closed" @if(old('comments_status') == 'closed') selected @endif>{{ __('admin/image.fields.closed') }}</option>
                                            </select>
                        ...
                        <div class="col-sm-12 col-md-10">
                            <button class="btn btn-primary bg-gray-800" type="submit">{{ __('admin/image.fields.create') }}</button>
                        </div>
                    </div>
                </form>
    ...
    @section('scripts')
        <script>
            ...
            $(document).on('click', '#gallery_table .add_watch', function(){
                var gallery_html = '';
                gallery_html += '<tr>';
                gallery_html += '<td><input type="text" placeholder="{{ __('admin/image.fields.gallery_link') }}" name="gallery[link][]" class="border-gray-300 rounded-md shadow-sm form-control" /></td>';
                gallery_html += '<td><button type="button" class="btn btn-danger btn-sm gallery_remove" style="background: red;"><span class="fa fa-minus"></span></button></td></tr>';
    ...
    @endsection
    @section('page_title'){{ __('admin/image.create.title_tag') }}@endsection
</x-admin-layout>
