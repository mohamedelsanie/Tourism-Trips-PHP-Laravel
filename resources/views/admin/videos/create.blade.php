<x-admin-layout>
                ...
                <div class="col-md-6 col-sm-12">
                    <nav aria-label="breadcrumb" role="navigation">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">{{ __('admin/video.create.create') }}</li>
                        </ol>
                    </nav>
                </div>
        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 text-gray-900">
                <div class="clearfix mb-10">
                    <div class="pull-left">
                        <h4 class="text-blue h4">{{ __('admin/video.create.create') }}</h4>
                    </div>
                </div>
                <div class="dropdown-divider"></div>

                <form action="{{ route('admin.videos.store') }}" method="post" class="mt-6 space-y-6">
                    @csrf
                    ...
                    <div class="form-group row">
                        <div class="col-sm-12 col-md-8 mb-30">
                                    ...
                                    <div class="tab-content">
                                        @foreach(config('translatable.languages') as $key => $lang)
                                            <div class="tab-pane @if(LaravelLocalization::getCurrentLocale() == $key) active @endif" id="title-{{ $key }}" role="tabpanel">
                                                <input name="{{ $key }}[title]" placeholder="{{ __('admin/video.fields.title') }}" value="{{ old($key.'.title') }}" class="@if(LaravelLocalization::getCurrentLocale() == $key) slug-input main_input @endif border-gray-300 rounded-md shadow-sm form-control @if($errors->has($key.'*title')) border border-danger @endif" type="text"/>
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-sm-12 col-md-12">
                                    <input name="slug" placeholder="{{ __('admin/video.fields.slug') }}" value="{{ old('slug') }}" class="slug-output border-gray-300 rounded-md shadow-sm form-control @error('slug') border border-danger @enderror" type="text"/>
                                            ...
                                            <div class="tab-content">
                                                @foreach(config('translatable.languages') as $key => $lang)
                                                    <div class="tab-pane @if(LaravelLocalization::getCurrentLocale() == $key) active @endif" id="description-{{ $key }}" role="tabpanel">
                                                        <textarea name="{{ $key }}[description]" placeholder="{{ __('admin/video.fields.description') }}" class="border-gray-300 rounded-md shadow-sm form-control @if($errors->has($key.'*description')) border border-danger @endif">{{ old($key.'.description') }}</textarea>
                                            ...
                                            <div class="tab-content">
                                                @foreach(config('translatable.languages') as $key => $lang)
                                                    <div class="tab-pane @if(LaravelLocalization::getCurrentLocale() == $key) active @endif" id="content-{{ $key }}" role="tabpanel">
                                                        <textarea name="{{ $key }}[content]" placeholder="{{ __('admin/video.fields.content') }}" id="editor_{{ $key }}" class="0textarea_editor_{{ $key }} border-gray-300 rounded-md shadow-sm form-control @if($errors->has($key.'*content')) border border-danger @endif">{{ old($key.'.content') }}</textarea>
                                ...
                                <div id="collapseImage" class="card-body show pb-0" data-parent="#accordionImage" aria-expanded="true">
                                    <div class="form-group row" id="user_image_field_{{$field}}">
                                        ...
                                        <div class="col-sm-12 col-md-12">
                                            ...
                                            <div class="block">
                                                <a href="#" class="btn-block" data-toggle="modal" data-target=".media_uploader_{{$field}}" type="button">{{ __('admin/video.fields.media') }}</a>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div class="form-group row">
                                        <div class="col-sm-12 col-md-12">
                                            <select name="status" class="border-gray-300 rounded-md shadow-sm custom-select col-12 @error('status') border border-danger @enderror">
                                                <option value="" @if(old('status') == '') selected @endif>{{ __('admin/video.fields.choose') }}</option>
                                                <option value="publish" @if(old('status') == 'publish') selected @endif>{{ __('admin/video.fields.publish') }}</option>
                                                <option value="pending" @if(old('status') == 'pending') selected @endif>{{ __('admin/video.fields.pending') }}</option>
                                                <option value="draft" @if(old('status') == 'draft') selected @endif>{{ __('admin/video.fields.draft') }}</option>
                                            </select>
                                    ...
                                    <div class="form-group row">
                                        <div class="col-sm-12 col-md-12">
                                            <select name="comments_status" class="border-gray-300 rounded-md shadow-sm custom-select col-12 @error('comments_status') border border-danger @enderror">
                                                ...
                                                <option value="open" @if(old('comments_status') == 'open') selected @endif>{{ __('admin/video.fields.open') }}</option>
                                                <option value="closed" @if(old('comments_status') == 'closed') selected @endif>{{ __('admin/video.fields.closed') }}</option>
                                            </select>
                        ...
                        <div class="col-sm-12 col-md-10">
                            <button class="btn btn-primary bg-gray-800" type="submit">{{ __('admin/video.fields.create') }}</button>
                        </div>
                    </div>
                </form>
    ...
    @endsection
    @section('page_title'){{ __('admin/video.create.title_tag') }}@endsection
</x-admin-layout>
