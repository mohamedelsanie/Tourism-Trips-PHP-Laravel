<x-admin-layout>
                ...
                <div class="col-md-6 col-sm-12">
                    <nav aria-label="breadcrumb" role="navigation">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">{{ __('admin/image_comment.edit.edit') }}<code>{{ $comment->title }}</code></li>
                        </ol>
                    </nav>
                </div>
        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 text-gray-900">
                <div class="clearfix mb-10">
                    <div class="pull-left">
                        <h4 class="text-blue h4">{{ __('admin/image_comment.edit.edit') }} </h4>
                    </div>
                </div>
                <div class="dropdown-divider"></div>

                <form action="{{ route('admin.image.comments.update', $comment->id) }}" method="post">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="form-group row">
                        <div class="col-sm-12 col-md-8 mb-30">
                            ...
                            <div class="form-group row">
                                <label class="col-sm-12 col-md-2 col-form-label">{{ __('admin/image_comment.fields.name') }}</label>
                                <div class="col-sm-12 col-md-10">
                                    <input name="name" placeholder="{{ __('admin/image_comment.fields.name') }}" value="{{ $comment->name }}" class="border-gray-300 rounded-md shadow-sm form-control @error('name') border border-danger @enderror" type="text"/>
                            ...
                            <div class="card card-box custom mb-10" id="accordionWork">
                                ...
                                <div id="collapseWork" class="card-body show" data-parent="#accordionWork" >
                                    ...
                                    <div class="form-group row">
                                        <label class="col-sm-12 col-md-2 col-form-label">{{ __('admin/image_comment.fields.comment') }}</label>
                                        <div class="col-sm-12 col-md-10">
                                    ...
                                    <div class="form-group row">
                                        <label class="col-sm-12 col-md-2 col-form-label">{{ __('admin/image_comment.fields.image_id') }}</label>
                                        <div class="col-sm-12 col-md-10">
                                            <select name="image_id" class="border-gray-300 rounded-md shadow-sm custom-select col-12 @error('image_id') border border-danger @enderror">
                                                <option value="" @if($comment->image_id == '') selected @endif>{{ __('admin/image_comment.fields.choose') }}</option>
                                                @foreach($posts as $post)
                                                    <option value="{{ $post->id }}" @if($comment->image_id == $post->id) selected @endif>{{ $post->title }}</option>
                                                @endforeach
                                            </select>
                                    ...
                                    <div class="form-group row">
                                        <label class="col-sm-12 col-md-2 col-form-label">{{ __('admin/image_comment.fields.user_id') }}</label>
                                        <div class="col-sm-12 col-md-10">
                                            <select name="user_id" class="border-gray-300 rounded-md shadow-sm custom-select col-12 @error('user_id') border border-danger @enderror">
                                                <option value="" @if($comment->user_id == '') selected @endif>{{ __('admin/image_comment.fields.choose') }}</option>
                                    ...
                                    <div class="form-group row">
                                        <label class="col-sm-12 col-md-2 col-form-label">{{ __('admin/image_comment.fields.comment_stars') }}</label>
                                        <div class="col-sm-12 col-md-10">
                                            <select name="comment_stars" class="border-gray-300 rounded-md shadow-sm custom-select col-12 @error('comment_stars') border border-danger @enderror">
                                                <option value="" @if($comment->comment_stars == '') selected @endif>{{ __('admin/image_comment.fields.choose') }}</option>
                                        ...
                                        <div class="col-sm-12 col-md-12">
                                            <select name="status" class="border-gray-300 rounded-md shadow-sm custom-select col-12 @error('status') border border-danger @enderror">
                                                @if($comment->status == 'publish')
                                                    <option value="publish" selected>{{ __('admin/image_comment.fields.publish') }}</option>
                                                    <option value="pending">{{ __('admin/image_comment.fields.pending') }}</option>
                                                    <option value="draft">{{ __('admin/image_comment.fields.draft') }}</option>
                                                @elseif($comment->status == 'pending')
                                                    <option value="publish">{{ __('admin/image_comment.fields.publish') }}</option>
                                                    <option value="pending" selected>{{ __('admin/image_comment.fields.pending') }}</option>
                                                    <option value="draft">{{ __('admin/image_comment.fields.draft') }}</option>
                                                @elseif($comment->status == 'draft')
                                                    <option value="publish">{{ __('admin/image_comment.fields.publish') }}</option>
                                                    <option value="pending">{{ __('admin/image_comment.fields.pending') }}</option>
                                                    <option value="draft" selected>{{ __('admin/image_comment.fields.draft') }}</option>
                                                @else
                                                    <option selected="">{{ __('admin/image_comment.fields.choose') }}</option>
                                                    <option value="publish">{{ __('admin/image_comment.fields.publish') }}</option>
                                                    <option value="pending">{{ __('admin/image_comment.fields.pending') }}</option>
                                                    <option value="draft">{{ __('admin/image_comment.fields.draft') }}</option>
                                                @endif
                        ...
                        <div class="col-sm-12 col-md-10">
                            <button class="btn btn-primary bg-gray-800" type="submit">{{ __('admin/image_comment.fields.update') }}</button>
                        </div>
                    </div>
                </form>
    ...
    @section('page_title'){{ __('admin/image_comment.edit.title_tag',['comment' => $comment->name]) }}@endsection
</x-admin-layout>
