<x-admin-layout>
                ...
                <div class="col-md-6 col-sm-12">
                    <nav aria-label="breadcrumb" role="navigation">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">{{ __('admin/page.edit.edit') }}<code>{{ $page->title }}</code></li>
                        </ol>
                    </nav>
                </div>
        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 text-gray-900">
                <div class="clearfix mb-10">
                    <div class="pull-left">
                        <h4 class="text-blue h4">{{ __('admin/page.edit.edit') }} </h4>
                    </div>
                </div>
                    ...
                    <div class="form-group row">
                        <div class="col-sm-12 col-md-8 mb-30">
                                    ...
                                    <div class="tab-content">
                                        @foreach(config('translatable.languages') as $key => $lang)
                                            <div class="tab-pane @if(LaravelLocalization::getCurrentLocale() == $key) active @endif" id="title-{{ $key }}" role="tabpanel">
                                                <input name="{{ $key }}[title]" placeholder="{{ __('admin/page.fields.title') }}" value="{{ $page->translate($key)->title }}" class="@if(LaravelLocalization::getCurrentLocale() == $key) slug-input main_input @endif border-gray-300 rounded-md shadow-sm form-control @if($errors->has($key.'*title')) border border-danger @endif" type="text"/>
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-sm-12 col-md-12">
                                    <input name="slug" placeholder="{{ __('admin/page.fields.slug') }}" value="{{ $page->slug }}" class="slug-output border-gray-300 rounded-md shadow-sm form-control @error('slug') border border-danger @enderror" type="text"/>
                                    <span class="">{{ __('admin/page.fields.current_slug') }}{{ getSetting('site_url') }}<b class="text-success">{{ $page->slug }}</b></span>
                                            ...
                                            <div class="tab-content">
                                                @foreach(config('translatable.languages') as $key => $lang)
                                                    <div class="tab-pane @if(LaravelLocalization::getCurrentLocale() == $key) active @endif" id="meta_description-{{ $key }}" role="tabpanel">
                                                        <input name="{{ $key }}[meta_description]" placeholder="{{ __('admin/page.fields.meta_desc') }}" value="{{ $page->translate($key)->meta_description }}" class="border-gray-300 rounded-md shadow-sm form-control @if($errors->has($key.'*meta_description')) border border-danger @endif" type="text"/>
                                            ...
                                            <div class="tab-content">
                                                @foreach(config('translatable.languages') as $key => $lang)
                                                    <div class="tab-pane @if(LaravelLocalization::getCurrentLocale() == $key) active @endif" id="meta_tags-{{ $key }}" role="tabpanel">
                                                        <input name="{{ $key }}[meta_tags]" placeholder="{{ __('admin/page.fields.meta_tags') }}" value="{{ $page->translate($key)->meta_tags }}" class="border-gray-300 rounded-md shadow-sm form-control @if($errors->has($key.'*meta_tags')) border border-danger @endif" type="text"/>
                                            ...
                                            <div class="tab-content">
                                                @foreach(config('translatable.languages') as $key => $lang)
                                                    <div class="tab-pane @if(LaravelLocalization::getCurrentLocale() == $key) active @endif" id="content-{{ $key }}" role="tabpanel">
                                                        <textarea name="{{ $key }}[content]" placeholder="{{ __('admin/page.fields.content') }}" id="editor_{{ $key }}" class="0textarea_editor_{{ $key }} border-gray-300 rounded-md shadow-sm form-control @if($errors->has($key.'*content')) border border-danger @endif">{{ $page->translate($key)->content }}</textarea>
                                ...
                                <div id="collapseImage" class="card-body show pb-0" data-parent="#accordionImage" aria-expanded="true">
                                    <div class="form-group row" id="user_image_field_{{$field}}">
                                        <div class="col-sm-6 col-md-6 hidden">
                                            <input name="image" id="user_image_{{$field}}" placeholder="image" value="{{ $page->image }}" class="hidden form-control @error('image') border border-danger @enderror" type="text"/>
                                        ...
                                        <div class="col-sm-12 col-md-12">
                                            <select name="status" class="border-gray-300 rounded-md shadow-sm custom-select col-12 @error('status') border border-danger @enderror">
                                                @if($page->status == 'publish')
                                                    <option value="publish" selected>{{ __('admin/page.fields.publish') }}</option>
                                                    <option value="pending">{{ __('admin/page.fields.pending') }}</option>
                                                    <option value="draft">{{ __('admin/page.fields.draft') }}</option>
                                                @elseif($page->status == 'pending')
                                                    <option value="publish">{{ __('admin/page.fields.publish') }}</option>
                                                    <option value="pending" selected>{{ __('admin/page.fields.pending') }}</option>
                                                    <option value="draft">{{ __('admin/page.fields.draft') }}</option>
                                                @elseif($page->status == 'draft')
                                                    <option value="publish">{{ __('admin/page.fields.publish') }}</option>
                                                    <option value="pending">{{ __('admin/page.fields.pending') }}</option>
                                                    <option value="draft" selected>{{ __('admin/page.fields.draft') }}</option>
                                                @else
                                                    <option selected="">{{ __('admin/page.fields.choose') }}</option>
                                                    <option value="publish">{{ __('admin/page.fields.publish') }}</option>
                                                    <option value="pending">{{ __('admin/page.fields.pending') }}</option>
                                                    <option value="draft">{{ __('admin/page.fields.draft') }}</option>
                                                @endif
                                        ...
                                        <div class="col-sm-12 col-md-12">
                                            <select name="comments_status" class="border-gray-300 rounded-md shadow-sm custom-select col-12 @error('comments_status') border border-danger @enderror">
                                                @if($page->comments_status == 'open')
                                                    <option value="open" selected>{{ __('admin/page.fields.open') }}</option>
                                                    <option value="closed">{{ __('admin/page.fields.closed') }}</option>
                                                @elseif($page->comments_status == 'closed')
                                                    <option value="open">{{ __('admin/page.fields.open') }}</option>
                                                    <option value="closed" selected>{{ __('admin/page.fields.closed') }}</option>
                                                @else
                                                    <option selected="">{{ __('admin/page.fields.choose') }}</option>
                                                    <option value="open">{{ __('admin/page.fields.open') }}</option>
                                                    <option value="closed">{{ __('admin/page.fields.closed') }}</option>
                        ...
                        <div class="col-sm-12 col-md-10">
                            <button class="btn btn-primary bg-gray-800" type="submit">{{ __('admin/page.fields.update') }}</button>
                        </div>
                    </div>
                </form>
    ...
    @endsection
    @section('page_title'){{ __('admin/page.edit.title_tag',['page' => $page->title]) }}@endsection
</x-admin-layout>
