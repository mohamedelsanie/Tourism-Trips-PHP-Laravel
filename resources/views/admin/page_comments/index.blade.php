<x-admin-layout>
                    ...
                    <nav aria-label="breadcrumb" role="navigation">
                        <ol class="breadcrumb">
                            ...
                            <li class="breadcrumb-item active" aria-current="page">
                                {{ __('admin/page_comment.index.title') }}
                            </li>
                        </ol>
                    </nav>
                </div>
                <div class="col-md-6 col-sm-12 text-right">
                    @if(AdminCan('pagecomment-create'))
                        <a href="{{ route('admin.page.comments.create') }}" class="btn btn-primary btn-sm scroll-click"><i class="fa fa-plus"></i> {{ __('admin/page_comment.index.create') }}</a>
                    @endif
                    @if(AdminCan('pagecomment-forcedelete'))
                        <a href="{{ route('admin.page.comments.archive') }}" class="btn btn-primary btn-sm scroll-click">{{ __('admin/page_comment.index.archive') }}</a>
                    @endif
                </div>
            </div>
        </div>
    ...
    <div class="py-12">
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 text-gray-900">
                <div class="row">
                    <div class="col-md-6 col-sm-12">
                        <div class="clearfix mb-10">
                            <div class="pull-left">
                                <h4 class="text-blue h4">{{ __('admin/page_comment.index.title') }}</h4>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="col-md-6 col-sm-12">
                        ...
                        <form name="search_form" method="post" action="{{route('admin.page.comments.search')}}" >
                            @csrf
                ...
                <form name="destroy_all" method="post" action="{{route('admin.page.comments.destroy_all')}}" >
                    @csrf
                    <table class="table table nowrap table-bordered table-striped no-footer dtr-inline">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="table-actions">
                                            @if(AdminCan('pagecomment-edit'))
                                                <a href="{{ route('admin.page.comments.edit',$comment->id) }}" data-color="#265ed7" style="color: rgb(38, 94, 215);"><i class="icon-copy dw dw-edit2"></i></a>
                                            @endif
                                            @if(AdminCan('pagecomment-delete'))
                                                ...
                                                <x-modal name="confirm-deletion_{{ $comment->id }}" focusable>
                                                    <div class="p-6">
                                                        ...
                                                        <div class="mt-6 flex justify-end">
                                                            ...
                                                            <a href="{{ route('admin.page.comments.delete',$comment->id) }}" class="inline-flex items-center px-4 py-2 bg-red-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-red-500 active:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 transition ease-in-out duration-150 ml-3">
                                                                {{ __('admin/common.messages.confirm_delete') }}
                                                            </a>
                                                        </div>
                                                    </div>
                                                </x-modal>
                                                {{--                                                <a href="{{ route('admin.page.comments.delete',$comment->id) }}" data-color="#e95959" style="color: rgb(233, 89, 89);"><i class="icon-copy dw dw-delete-3"></i></a>--}}
                                            @endif
                                        </div>
                                    </td>
                                </tr>
                        ...
                        @else
                            <tr>
                                <td colspan="5">{{ __('admin/page_comment.index.notfound') }}</td>
                            </tr>
                        @endif
                        </tbody>
                    </table>
    ...
    @section('page_title'){{ __('admin/page_comment.index.title_tag') }}@endsection
</x-admin-layout>
