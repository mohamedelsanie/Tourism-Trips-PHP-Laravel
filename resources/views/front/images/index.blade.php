            ...
            <div class="row h-100 align-items-center">
                <div class="col-12">
                    <div class="breadcrumb-content text-center">
                        <h2 class="page-title">{{ __('front/post_types.photos.title') }}</h2>
                        <nav aria-label="breadcrumb">
                            <ol class="breadcrumb justify-content-center">
                                <li class="breadcrumb-item"><a href="{{ route('homepage') }}">{{ __('front/common.home') }} </a></li>
                                <li class="breadcrumb-item active" aria-current="page">{{ __('front/post_types.photos.title') }}</li>
                            </ol>
                        </nav>
                    </div>
                </div>
...
<div class="roberto-news-area section-padding-100-0">
                ...
                @foreach($posts as $post)
                    <div class="single-blog-post d-flex align-items-center mb-50 wow fadeInUp" data-wow-delay="100ms">
                        <div class="post-thumbnail">
                            <a href="{{ getImageLink($post->slug) }}"><img src="{{ $post->image }}" alt="{{ $post->title }}"></a>
                        </div>
                        <div class="post-content">
                            <div class="post-meta">
                                <a href="{{ getImageLink($post->slug) }}" class="post-author">{{ $post->created_at->format('Y-m-d') }}</a>
                            </div>
                            <a href="{{ getImageLink($post->slug) }}" class="post-title">{{ $post->title }}</a>
                            <p>{!! Str::words($post->description, 20,'...') !!}</p>
                            <a href="{{ getImageLink($post->slug) }}" class="btn continue-btn">{{ __('front/post_types.photos.view_post') }}</a>
                        </div>
                    </div>
                    @endforeach
    ...
    <br />
    <br />
    @section('page_title'){{ __('front/post_types.photos.title') }}@endsection
</x-app-layout>